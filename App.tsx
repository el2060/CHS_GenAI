    ...
    return (
        <div className="relative w-full h-full flex flex-col items-center justify-center text-center p-8">
            <div className="flex flex-col items-center justify-center">
                ...
                <h2 className={`text-6xl md:text-7xl font-light text-primary mt-6 ${isFirstSlide ? 'fade-in' : ''}`} style={{animationDelay: isFirstSlide ? '0.2s' : '0s'}}>
                    {slide.subtitle}
                </h2>
            </div>
            {isFirstSlide && (slide.footer) && (
                <div className="absolute bottom-8 text-center w-full">
                    {slide.footer && <p className="text-2xl text-muted-foreground fade-in" style={{ animationDelay: '1.6s' }}>{slide.footer}</p>}
                </div>
            )}
        </div>
    );
    ...
    <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-3 gap-8">
      {(slide.columns as ThreeColumnItem[])?.map((col, index) => (
        <div key={index} className="bg-card border border-border rounded-2xl p-8 flex flex-col items-center text-center transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
          <span className="material-icons-outlined text-8xl mb-6" style={{ color: col.iconColor || '#3b82f6' }}>
            {col.icon}
          </span>
          <h3 className="text-3xl font-semibold text-card-foreground leading-snug">{col.title}</h3>
        </div>
      ))}
    </div>
  </div>
    ...
    <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8 self-center">
      {(slide.columns as ComparisonColumn[])?.map((col, index) => (
        <div key={index} className={`flex flex-col text-center items-center p-8 rounded-2xl border-2 ${index === 1 ? 'border-primary bg-blue-50/50' : 'border-border bg-card'}`}>
          ...
          <h3 className="text-5xl font-semibold text-foreground">{col.title}</h3>
          <p className="text-2xl text-muted-foreground mt-1">{col.chineseTitle}</p>
          <p className="text-4xl font-medium text-foreground my-8">{col.text}</p>

          <div className="bg-muted rounded-lg p-6 mt-auto w-full">
            <p className="text-3xl font-medium text-foreground italic">"{col.example}"</p>
...
const ChallengeSlide: React.FC<{ slide: SlideData }> = ({ slide }) => (
  <div className="w-full h-full flex flex-col items-center justify-center text-center p-8 fade-in">
    <span className="material-icons-outlined text-[12rem] mb-12" style={{ color: slide.iconColor || '#3b82f6' }}>
      {slide.icon}
    </span>
      ...
      {(slide.cardItems as CardListItem[])?.map((item, index) => (
        <div key={index} className="bg-card border border-border rounded-2xl p-6 shadow-sm flex items-start transition-all duration-300 hover:shadow-lg hover:border-primary">
          ...
          <div className="w-3/4 pl-6 border-l border-border">
            <p className="text-3xl text-foreground">{item.text}</p>
  ...
  <div className="w-full h-full flex flex-col items-center justify-center p-4">
    ...
    <div className="w-full max-w-8xl grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 h-full">
        <h3 className="text-3xl font-semibold text-primary text-center mb-2">{slide.infographicSections?.[0].title}</h3>

        <div className="space-y-3">
          {slide.infographicSections?.[0].items.map((item, index) => (
            <div key={index} className="flex items-center bg-muted/50 p-3 rounded-lg">
              <span className="material-icons-outlined text-6xl mr-4" style={{ color: item.iconColor }}>{item.icon}</span>
              <div>
      ...
      <div className="bg-card border-2 border-primary rounded-2xl p-6 shadow-lg">
        <h3 className="text-3xl font-semibold text-primary text-center mb-2">Business Applications</h3>
        ...
        <div className="grid grid-cols-3 gap-x-4 gap-y-2 text-center">
          {(slide.applications as InfographicApplication[])?.map((app, index) => (
            <div key={index} className="flex flex-col items-center p-1 rounded-lg hover:bg-muted transition-colors duration-200">
              ...
              <p className="text-2xl font-medium text-card-foreground mt-1">{app.name}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-red-50/50 border border-red-200 rounded-2xl p-6 h-full">
         <h3 className="text-3xl font-semibold text-accent-red text-center mb-2">{slide.infographicSections?.[1].title}</h3>
        ...
        <div className="bg-card border border-border rounded-2xl p-4">
            ...
            {(slide.workflow as WorkflowStep[]).map((step, index) => (
                ...
                <div className="flex flex-col items-center px-2">
                    ...
                    <p className="mt-1 text-xl font-semibold text-foreground">{step.label}</p>

                </div>
                {index < slide.workflow.length - 1 && (
                    <span className="material-icons-outlined text-5xl text-muted-foreground/70 mx-2">arrow_forward</span>
                )}
                </React.Fragment>
            ))}
            </div>
...
const AgendaSlide: React.FC<{ slide: SlideData }> = ({ slide }) => (
  <div className="relative w-full h-full flex flex-col items-center justify-center p-12">
    ...
    <div className="text-center mb-12">
      <h1 className="text-6xl md:text-7xl font-bold text-foreground tracking-tight">{slide.title}</h1>
      <h2 className="text-4xl md:text-5xl font-medium text-muted-foreground mt-2">{slide.subtitle}</h2>
    </div>
      ...
      {(slide.agendaColumns as AgendaColumn[])?.map((col, index) => (
        <React.Fragment key={index}>
          ...
          >
            ...
            {col.isOrdered ? (
              <ol className="list-decimal list-inside space-y-3 text-xl text-foreground/90 pl-2">
                {col.items.map((item, itemIndex) => (
                  <li key={itemIndex}>{item.text}</li>
                ))}
              </ol>
            ) : (
              <ul className="list-disc list-inside space-y-3 text-xl text-foreground/90 pl-2">
                {col.items.map((item, itemIndex) => (
                  <li key={itemIndex}>{item.text}</li>
                ))}
              </ul>
          ...
          {index === 0 && (slide.agendaColumns?.length ?? 0) > 1 && (
            <div className="flex items-center justify-center text-7xl text-muted-foreground/40 font-light px-4">
              →
            </div>
          )}
        </React.Fragment>
    ...
    {slide.footer && (
        <div className="mt-12 text-center w-full max-w-7xl">
            <p className="text-3xl text-muted-foreground/80 italic">
    ...
    <div className="w-full h-full flex flex-col items-center justify-center text-center p-8">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 shadow-lg mb-8 w-full max-w-5xl flex items-center space-x-8">
        <div className="w-64 h-48 flex-shrink-0 flex items-center justify-center bg-muted rounded-lg border border-border">
            <span className="material-icons-outlined text-[10rem] text-primary">{quizIcon}</span>
        </div>
        <p className="text-4xl font-medium text-foreground text-left flex-1 leading-snug">{scenario}</p>
      </div>

      <div className="flex items-center justify-center gap-8 w-full max-w-5xl">
        {CHOICES.map(choice => {
          let buttonClasses = "text-4xl font-semibold py-8 px-16 rounded-full border-4 transition-all duration-300 w-1/2 flex items-center justify-center gap-4 ";
          if (isAnswerSlide) {
          ...
          return <button key={choice} className={buttonClasses} disabled={isAnswerSlide}>
            {choice === 'Traditional AI' ? <span className="material-icons-outlined text-5xl">search</span> : <span className="material-icons-outlined text-5xl">auto_awesome</span>}
            {choice}
          </button>;
        })}
      </div>
      ...
      {isAnswerSlide && (
        <div className="mt-10 bg-blue-50 border-2 border-primary/30 rounded-2xl p-6 w-full max-w-5xl text-left flex items-start fade-in" style={{ animationDelay: '0.2s' }}>
          <span className="material-icons-outlined text-6xl text-primary mr-4">lightbulb</span>
          <div>
            <h3 className="text-4xl font-bold text-primary">{correctAnswer}</h3>
            <p className="text-3xl text-foreground mt-1">{renderFooterSpans(explanation)}</p>
          </div>
        </div>
      )}
    </div>
    ...
    <div className={`${baseClasses} ${activeClasses}`}>
      ...
      {hasFooter && (
                <footer className="absolute bottom-10 md:bottom-12 w-full text-center">
                  <p className="text-3xl md:text-4xl text-muted-foreground">{renderFooterSpans(slide.footer)}</p>
                </footer>
      )}
    </div>
  );
...
const App: React.FC = () => {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-background">
      ...
      {/* Bottom Bar */}
      <div className="absolute bottom-0 left-0 right-0 h-12 px-6 z-20 flex items-center justify-end">
        <div className="text-xl text-muted-foreground font-medium bg-muted px-3 py-1 rounded-md">
